...
public class CategoryAppsActivity extends Activity {
    ...
    protected void onCreate(Bundle b) {
        ...
        if (btnLoadMore != null) btnLoadMore.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                int perPage = Prefs.getPerPage(CategoryAppsActivity.this);
                visibleCount += perPage;
                applySort();
            }
        });
        if (btnTopFree != null) btnTopFree.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) { sortDownloads = false; visibleCount = Prefs.getPerPage(CategoryAppsActivity.this); applySort(); updateTabButtons(); }
        });
        if (btnTopDownloads != null) btnTopDownloads.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) { sortDownloads = true; visibleCount = Prefs.getPerPage(CategoryAppsActivity.this); applySort(); updateTabButtons(); }
        });
    ...
    private void loadApps(final String category, final boolean isGame) {
        ...
        new AsyncTask<Void, Void, ArrayList<AppItem>>() {
            ...
            protected void onPostExecute(ArrayList<AppItem> out) {
                ...
                visibleCount = Prefs.getPerPage(CategoryAppsActivity.this);
                bindPromotion();
                applySort();
                updateTabButtons();
            }
    ...
    private void applySort() {
        ArrayList<AppItem> sorted = new ArrayList<AppItem>(originalItems);
        if (sortDownloads) {
            Collections.sort(sorted, new Comparator<AppItem>() {
        ...
        items.clear();
        int end = Math.min(visibleCount, sorted.size());
        for (int i = 0; i < end; i++) items.add(sorted.get(i));
        if (btnLoadMore != null) {
            boolean hasMore = visibleCount < sorted.size();
